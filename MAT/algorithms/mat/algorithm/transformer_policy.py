...
class TransformerPolicy:
    ...
    def get_actions(self, cent_obs, obs, available_actions=None,
        ...
        # actions和action_log_probs的维度是(batch_size, num_agents, 1)
        actions, action_log_probs, values = self.transformer.get_actions(cent_obs,
                                                                         obs,
                                                                         available_actions,
                                                                         deterministic)

        actions = actions.view(-1, self.act_num)  # 此时actions的维度是(batch_size*num_agent, self.act_num)
    ...
    def evaluate_actions(self, cent_obs, obs, actions, available_actions=None, active_masks=None):
        """
