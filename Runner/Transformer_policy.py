...
import datetime
import numpy as np
from Runner.util import check
from Runner.MAT import MultiAgentTransformer as MAT
from Runner.MAT import Critic
...
from Runner.transformer_policy_withoutShare import TransformerPolicy as Policy
...
class TransformerPolicy:
    ...
    def __init__(self, args, num_agents, device=torch.device("cpu")):
        ...
        # self.share_obs_dim = get_shape_from_obs_space(cent_obs_space)[0]  # 一个智能体的Discrete action space，它有一个属性n，表示动作的个数
        ...
        self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.reasoning_dim, self.act_dim, num_agents,
        ...
        self.Critic = Critic(self.share_obs_dim, self.obs_dim, args.hidden_size, self.act_dim, args.n_block, args.n_embd, args.n_head, self.num_agents, args.encode_state, device=device)
    ...
    def get_actions(self, cent_obs, obs, available_actions,
                    deterministic=False):
        """
    ...
    def get_values2(self, cent_obs, obs, masks, available_actions):
        """
        Get value function predictions.
        :param cent_obs (np.ndarray): centralized input to the critic.
        :param rnn_states_critic: (np.ndarray) if critic is RNN, RNN states for critic.
        ...
        values = self.Critic.get_values(cent_obs, obs)
    ...
    def evaluate_actions(self, cent_obs, obs, actions, masks, available_actions, active_masks=None):
        """
    ...
    def act(self, cent_obs, obs, available_actions, deterministic=True):
        """
